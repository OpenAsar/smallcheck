...
const indexCode = `const native = require('./cld.node');
...
export default (original, out) => {
  writeFileSync(join(out, 'index.js'), indexCode);
  copyFileSync(join(original, 'node_modules', 'cld', 'build', 'Release', 'cld.node'), join(out, 'cld.node'));
};